...
extern void single_dmp_test_derivatives();

// This function updates continuous states using the ODE8 fixed-step
//  solver algorithm

void untitledModelClass::rt_ertODEUpdateContinuousStates(RTWSolverInfo *si )
{
  // Solver Matrices
#define rtNSTAGES                      13
...
void untitledModelClass::step()
{
  ...
  // Outport: '<Root>/position_current' incorporates:
  //   Integrator: '<Root>/left'

  rtY.position_current[0] = rtX.left_CSTATE[0];
  ...
  rtY.position_current[1] = rtX.left_CSTATE[1];
  ...
  rtY.position_current[2] = rtX.left_CSTATE[2];
  ...
  for (k = 0; k < 3; k++) {
    ...
    rtDW.Gain27[k] = ((((rtConstP.Constant2_Value[k] - rtX.left_CSTATE[k]) *
                        1050.0 + -194.421 * rtDW.x3[k]) + tmp[k] *
                       rtX.Integrator2_CSTATE / a * 1050.0) + rtConstB.Gain30[k]
                      * rtX.Integrator2_CSTATE) * 0.1111111111111111;
  }
...
void untitledModelClass::single_dmp_test_derivatives()
{
  untitledModelClass::XDot *_rtXdot;
...
untitledModelClass::RT_MODEL * untitledModelClass::getRTM()
{
  return (&rtM);
}
